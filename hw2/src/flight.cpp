...
Flight::Flight(const string flightNumber,
	...
	rows(rows), seatsPerRow(seatsPerRow), aisles(aisles), classBounds(classBounds) {
...
void Flight::assignSeat(const string& passengerName,
	...
	if (logConfirmation) {
		// print confirmation
		cout << "Seat " << row + 1 << seatNumberToLetter(seat) << " has been assigned to "
			 << passengerName << endl;
	}
}
