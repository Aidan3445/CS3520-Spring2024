...
template<class T>
CommaSeparatedList<T>::CommaSeparatedList(T* d, int s) {  // match data type to T*
	data = d;
	size = s;
}

template<class T>
void CommaSeparatedList<T>::printList() {	  // < T ≤ -> <T>
...
int main() {
	...
	CommaSeparatedListSize = sizeof(someBook) / sizeof(someBook[0]);  // sizeif -> sizeof
	...
	CommaSeparatedListOfBooks.printList();	// prinList -> printList
