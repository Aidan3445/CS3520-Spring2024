...
template<class T>
Calculator<T>::Calculator(T n1, T n2) {	 // int -> T
	num1 = n1;
	num2 = n2;
}

template<class T>
T Calculator<T>::add() {  // add return type T
	return num1 + num2;
}

template<class T>
T Calculator<T>::subtract() {  // add return type T
	return num1 - num2;
}

template<class T>
T Calculator<T>::multiply() {  // remove params
	return num1 * num2;		   // use class members
}
...
template<class T>
bool Calculator<T>::isgreater() {  // return type bool
	if (num1 > num2)
		return true;
	else
		return false;
...
int main() {
	...
	// string operations
	Calculator<string> calc3("Hello", "Goodbye");  // 1 -> "goodbye" (string)
	cout << calc3.add() << endl;
