...
std::vector<ProjectTeam*> GroupCreator::preferentialTeams(std::vector<StudentPref*> students) {
	...
	for (StudentPref* student : students) {
		...
		if (student->team == nullptr) {
			...
			auto validTeam =
				std::find_if(teams.begin(), teams.end(), [student](const ProjectTeam* team) {
			...
			for (StudentPref* pref : student->preferredPartners) {
				...
				if (pref->team == nullptr && !team->isRejected(pref) &&
					!pref->isRejected(student)) {
	...
	while (revIt != teams.rend()) {
		...
		for (auto student : team->members) {
			if (!(*sortedTeams.begin())->isRejected(student)) {
				...
				// re-sort the teams
				std::sort(
					sortedTeams.begin(), sortedTeams.end(), [](ProjectTeam* a, ProjectTeam* b) {
...
std::vector<ProjectTeam*> GroupCreator::balanceTeams(std::vector<ProjectTeam*> teams,
													 Comparator comp,
													 int maxIterations,
													 double maxDelta) {
		...
		for (StudentPref* student : lowest->members) {
			...
			for (auto it = teamSet.rbegin(); it != teamSet.rend(); it++) {
				...
				for (auto it2 = highest->members.rbegin(); it2 != highest->members.rend(); it2++) {
					...
					if (!highest->isRejected(student) && !lowest->isRejected(student2) &&
						comp.studentComp(student, student2)) {
						// swap the students
						student->team = highest;
						student2->team = lowest;
						highest->members.push_back(student);
						lowest->members.push_back(student2);
						highest->members.erase(
							std::find(highest->members.begin(), highest->members.end(), student2));
						lowest->members.erase(
							std::find(lowest->members.begin(), lowest->members.end(), student));

						// re-sort the teams
						std::sort(
							highest->members.begin(), highest->members.end(), comp.studentComp);
						std::sort(lowest->members.begin(), lowest->members.end(), comp.studentComp);
